 ...
 * @branch  feat/tenant-integration-tests
 */
...
describe('IntegrationTests - integration-tests', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe('findById', () => {
    it('returns the record when it exists', async () => {
      ...
      (prisma.lease.findUnique as jest.Mock).mockResolvedValue(mockIntegrationTests);

      // Act & Assert
      // TODO: call actual service/function and assert result
      expect(mockIntegrationTests.id).toBe('test-id-123');
    });
  ...
  describe('create', () => {
    it('creates a new record with valid input', async () => {
      // Arrange
      const { prisma } = await import('@/lib/prisma');
      (prisma.lease.create as jest.Mock).mockResolvedValue(mockIntegrationTests);

      // Act & Assert
      // TODO: call create and verify the returned object
      expect(mockIntegrationTests).toHaveProperty('id');
    });
  ...
  describe('update', () => {
    it('updates an existing record', async () => {
      const updated = { ...mockIntegrationTests, updatedAt: new Date() };
      const { prisma } = await import('@/lib/prisma');
      (prisma.lease.findUnique as jest.Mock).mockResolvedValue(mockIntegrationTests);
  ...
  describe('delete', () => {
    it('deletes an existing record', async () => {
      const { prisma } = await import('@/lib/prisma');
      (prisma.lease.findUnique as jest.Mock).mockResolvedValue(mockIntegrationTests);
      (prisma.lease.delete as jest.Mock).mockResolvedValue(mockIntegrationTests);

      // TODO: call delete and assert no error thrown
      expect(mockIntegrationTests.id).toBeDefined();
    });
  });
});
