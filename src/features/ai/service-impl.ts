...
export interface CreateServiceImplDto {
  // TODO: add creation fields
  [key: string]: unknown;
}

export interface UpdateServiceImplDto {
  // TODO: add update fields
  [key: string]: unknown;
}

export class ServiceImplService {
  private readonly log = logger.child({ service: 'ServiceImplService' });
  ...
  async create(dto: CreateServiceImplDto): Promise<Ai> {
    this.log.info({ dto }, 'create');
    return prisma.ai.create({ data: dto as never });
  }

  async update(id: ID, dto: UpdateServiceImplDto): Promise<Ai> {
...
export const serviceImplService = new ServiceImplService();
