/**
 * @module  tenant
 * @feature lease-service
 * @branch  feat/tenant-lease-service
 */
...
export interface CreateLeaseServiceDto {
  // TODO: add creation fields
  [key: string]: unknown;
}

export interface UpdateLeaseServiceDto {
  // TODO: add update fields
  [key: string]: unknown;
}

export class LeaseServiceService {
  private readonly log = logger.child({ service: 'LeaseServiceService' });
  ...
  async create(dto: CreateLeaseServiceDto): Promise<Lease> {
    this.log.info({ dto }, 'create');
    return prisma.lease.create({ data: dto as never });
  }

  async update(id: ID, dto: UpdateLeaseServiceDto): Promise<Lease> {
...
export const leaseServiceService = new LeaseServiceService();
