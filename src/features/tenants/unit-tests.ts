...
describe('UnitTests - unit-tests', () => {
  ...
  describe('findById', () => {
    it('returns the record when it exists', async () => {
      ...
      (prisma.lease.findUnique as jest.Mock).mockResolvedValue(mockUnitTests);
  ...
  describe('create', () => {
    it('creates a new record with valid input', async () => {
      // Arrange
      const { prisma } = await import('@/lib/prisma');
      (prisma.lease.create as jest.Mock).mockResolvedValue(mockUnitTests);
  ...
  describe('delete', () => {
    it('deletes an existing record', async () => {
      const { prisma } = await import('@/lib/prisma');
      (prisma.lease.findUnique as jest.Mock).mockResolvedValue(mockUnitTests);
      (prisma.lease.delete as jest.Mock).mockResolvedValue(mockUnitTests);
