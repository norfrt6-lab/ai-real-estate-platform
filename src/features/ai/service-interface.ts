 ...
 * @branch  feat/ai-service-interface
 */
...
export interface CreateServiceInterfaceDto {
  // TODO: add creation fields
  [key: string]: unknown;
}

export interface UpdateServiceInterfaceDto {
  // TODO: add update fields
  [key: string]: unknown;
}

export class ServiceInterfaceService {
  private readonly log = logger.child({ service: 'ServiceInterfaceService' });
  ...
  async create(dto: CreateServiceInterfaceDto): Promise<Ai> {
    this.log.info({ dto }, 'create');
    return prisma.ai.create({ data: dto as never });
  }

  async update(id: ID, dto: UpdateServiceInterfaceDto): Promise<Ai> {
...
export const serviceInterfaceService = new ServiceInterfaceService();
