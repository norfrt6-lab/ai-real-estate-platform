/**
 * @module  ai
 * @feature chatbot-backend-service
 * @branch  feat/ai-chatbot-backend-service
 */
...
export interface CreateChatbotBackendServiceDto {
  // TODO: add creation fields
  [key: string]: unknown;
}

export interface UpdateChatbotBackendServiceDto {
  // TODO: add update fields
  [key: string]: unknown;
}

export class ChatbotBackendServiceService {
  private readonly log = logger.child({ service: 'ChatbotBackendServiceService' });
  ...
  async create(dto: CreateChatbotBackendServiceDto): Promise<Ai> {
    this.log.info({ dto }, 'create');
    return prisma.ai.create({ data: dto as never });
  }

  async update(id: ID, dto: UpdateChatbotBackendServiceDto): Promise<Ai> {
...
export const chatbotBackendServiceService = new ChatbotBackendServiceService();
